...
<h2><?php {p($page["contents_name"]);} ?></h2>
    <?php
        ...
        p('<a href="'.$local_root.'/admin/">管理トップへ</a>');
        p('　　<a href="'.$local_root.'/admin/'.$entity_name.'/create/">新規登録はこちら</a>');
    ...
    <?php
        foreach(getEntityList($table_name) as $q){
            ...
            p('<td><a href="'.$local_root.'/admin/'.$entity_name.'/update/'.$key_parameter.'">更新</a></td>');
            p('<td><a href="'.$local_root.'/admin/'.$entity_name.'/delete/'.$key_parameter.'">削除</a></td>');
            foreach(getColumnList($table_name) as $c){
